function warning(a){console.warning("%c "+a,"color:white;font-weight:bold;background-color:#de5b00;font-size:20px;")}function bad(a){console.error("%c "+a,"color:white;font-weight:bold;background-color:red;font-size:20px;")}function good(a){console.error("%c "+a,"color:white;font-weight:bold;background-color:green;font-size:20px;")}
...
window.__XEN_WEBPACK.core.AppManagerComponent = class AMC {
  ...
  async install(pkg, repo = "https://xenos-app-repository.enderkingj.repl.co") {
    ...
    for (let asset of meta.assets) {
      ...
      await this.#install(author, proj, asset, body, meta.entry);
    }
  ...
  async open(pkg, callbackFunc, openType) {
    ...
    if (meta.type=='web') {
      ...
      if (openType === "reg") {
        xen.system.register(meta.name, "10", "10", path + "/index.html", true);
        bad("WARNING! You're using a deprecated method, we advise using app.open");
      } 
      else {
